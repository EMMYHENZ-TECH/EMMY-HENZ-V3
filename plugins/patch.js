  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`EMMY_HENZ-V3- UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
${menuThemeFooter}                         
*┌┤✑  Thanks for Choosing EMMY_HENZ-V3
│└────────────┈ ⳹        
│*©2024-2099 EMMY-HENZ*
└─────────────────┈ ⳹*
${readmore}`;
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*EMMY_HENZ-V3 ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `EMMY_HENZ-V3 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\nEMMY_HENZ-V3`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*EMMY_HENZ-V3 ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "EMMY_HENZ-V3 Running Since",
        body: message,
        footerText: "EMMY_HENZ-V3 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("EMMY_HENZ-V3_FANCY_TEXT")
        ) {
