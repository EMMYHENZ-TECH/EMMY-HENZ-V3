const {
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*EMMY_HENZ-V3 ʙᴀʀᴅ ᴀɪ:*\n ${result}`, { quoted: message });
      } catch (error) {
  ...
  smd({
    ...
    try {
      ...
      const { result } = res;
      const astro = "EMMY_HENZ-V3 ɢᴇᴍɪɴɪ ᴀɪ"
      const tbl = "```";
  ...
  smd(
    ...
    async (_0x1c0160, _0x482db1) => {
        let astro = "EMMY_HENZ-V3 ᴄʜᴀᴛ ʙᴏᴛ"
      try {
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("EMMY_HENZ-V3• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("EMMY_HENZ-V3 • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*EMMY_HENZ-V3 • ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
